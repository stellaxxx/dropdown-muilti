...
var common = {
	...
	/**
	 * url을 "container"라는 ID를 가진 영역으로 불러들인다.
	 * 오류시 container영역에 오류메세지가 나타난다.
	 */
	go : function(url) {
	...
	/**
	 * url을 target ID를 가진 영역으로 불러들인다. 오류시
	 * target 영역에 오류메세지가 나타난다.
	 */
	goUrl : function(url, target, data) {
		...
		$.ajax({
			...
			error : function(xhr, status, e) {
				...
				var resultTxt = xhr.responseText.match(/<h1>(.*)<\/h1>/gm)[0];
				resultTxt = resultTxt.replace(/(<([^>]+)>)/ig, "");
				resultTxt = "<div class=\"container\">"
						+ "<div class=\"alert alert-danger\">" + resultTxt
						+ "</div></div>";
	...
	pagination : function( fnName, totalCount, currentPageStartIndex, showRowCount ) {
		...
		//총 페이지 개수
		var totalPage = parseInt(totalCount / rowCount);
